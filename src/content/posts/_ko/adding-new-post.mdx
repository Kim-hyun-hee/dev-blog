...
Keep the `>` marker on every line, including the fences, and a code block works
inside a callout like anywhere else — highlighting, the copy button, and the
`file=` label all come along:

> [!NOTE] Restart after editing taxonomy
> Editing `src/categories.ts` or `src/series.ts` needs a dev server restart.
> Otherwise the sidebar keeps showing stale counts.
>
> ```bash file="terminal"
> corepack pnpm astro dev stop
> ```
>
> ```bash
> corepack pnpm dev
...
Inside a callout the frame is quieter than in body text: the traffic lights,
the language badge and the drop shadow are dropped, and the header row survives
only when you name the file. Body-level code blocks keep the full frame.
